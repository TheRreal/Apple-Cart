...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderProducts(products) {
        ...
        products.forEach(product => {
            ...
            `; // ** First $ is acutal Dollar sign then second "$" is apart of interpolation inside of `...` 
              // {inserting variables or expressions into a string using template literals (backticks) }
    ...
    function attachAddToCartListeners() { // ** function sets up click event listeners on each "Add to Basket" button
        document.querySelectorAll(".store__add-to-cart").forEach(button => {
            button.addEventListener("click", function () {
                ...
                const existingProduct = cart.find(item => item.id === productId); //=== (strict equality) does no type conversion — that's why it’s considered safer and more predictable.
                ...
                updateCart();
                showToast(`${productName} added to basket`, 'success'); // function displays temporary popup messages (toasts) on the page to quickly notify users about actions they just performed, like adding items to the cart or clearing the basket
            });
        });
    }
    
    // Function to attach event listeners to "Add to Basket" buttons
    function updateCart() { //  ** function syncs the current shopping cart data with both the user interface and the browser's localStorage
    ...
    function updateCartPreview() { //  ** function syncs the current shopping cart data with both the user interface and the browser's localStorage
        ...
        cart.forEach((item, index) => {
            ...
            total += item.price * item.quantity; // ** JavaScript first multiplies item.price by item.quantity and then adds the result to the current value 
                                                // ** of total, assigning the final sum back to total; multiplication has higher precedence than addition assignment 
        });

        cartTotalElement.textContent = total.toFixed(2); // ** Converts product.price to a string with two decimal places, suitable for showing as a price.

    }

    // Function to clear the basket
    function clearCart() { // Empties shopping cart, updates cart display; is tied to "Clear Basket" button in preview section the also updates local storage on user browser.
        cart = [];
        updateCart();
    ...
    renderProducts(products); // Sifts through the products.js to display all the items on the webpage.
    updateCart();
